...
function WomanShoes() {
  ...
  // Filter products to get only the ones with category "Woman Shoes"
  const womanShoesProducts = ProductsData.filter(product => product.category === "Woman Shoes");
...
// This variable holds the list of man trainers products after applying the selected color and brand filters.
  const filteredProducts = womanShoesProducts.filter(product => {
...
// Called when the user selects a sorting option from the dropdown. It sets the sorting order based on the selected option.
  ...
  const categoryToShow = womanShoesProducts.length > 0 ? womanShoesProducts[0].category : '';
